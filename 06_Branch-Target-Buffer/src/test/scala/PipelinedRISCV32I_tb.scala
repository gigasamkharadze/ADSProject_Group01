...
"RV32I_BasicTester" should "work" in {
  ...
  // ── Control Hazard Regression Tests (ported unmodified from Directory 5) ──
  // Each of these branches executes exactly once, so the BTB (enabled by default,
  // useBTB = true) never has a valid entry for it yet: IF's lookup misses, falls
  // back to the static "assume not taken" prediction, and EX's flush/redirect
  // behaviour is therefore byte-for-byte identical to the Directory 5 static core.
