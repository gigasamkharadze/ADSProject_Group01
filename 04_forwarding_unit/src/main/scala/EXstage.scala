...
class EX extends Module {
  val io = IO(new Bundle {
    ...
    val exBarrier_data = Input(UInt(32.W))
    val memBarrier_data = Input(UInt(32.W))
  ...
  when(io.fwd_A_sel === 1.U)      { forwardedA := io.exBarrier_data }
  .elsewhen(io.fwd_A_sel === 2.U) { forwardedA := io.memBarrier_data }
  when(io.fwd_B_sel === 1.U)      { forwardedB := io.exBarrier_data }
  .elsewhen(io.fwd_B_sel === 2.U) { forwardedB := io.memBarrier_data }
